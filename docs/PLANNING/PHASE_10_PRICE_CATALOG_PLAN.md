...
[PHASE_10_llm_infra_plan.md](PHASE_10_llm_infra_plan.md).
...
2. Обязан вернуть **контрактный (constrained) ответ**: для категории — либо точное совпадение
   ...
   `PHASE_10_llm_infra_plan.md` — свободный текст без схемы даёт разнобой вроде «малярные
...
Исходная инфраструктура (`PHASE_10_llm_infra_plan.md`) сознательно **не даёт LXC 107/108
...
для `PHASE_10_llm_infra_plan.md` (там же, где уже есть таблица замеров токенов/сек по моделям).
