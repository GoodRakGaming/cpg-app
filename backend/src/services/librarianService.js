...
// «Умный дом» с «Электромонтажные работы»). Вербатим-копия CATEGORY_DESCRIPTIONS из
// docs/PLANNING/test_librarian_real_data_round5.py — тем самым скриптом реально валидировался
// результат 0/31 (round 5). Часть описаний подтверждена владельцем продукта напрямую по xlsx
// (см. комментарии в самом скрипте), часть дозаполнена тестировщиком по принципу «категория =
// специальность бригады-исполнителя, а не физический объект/материал работы».
const CATEGORY_DESCRIPTIONS = {
  'Демонтажные работы': 'демонтаж, резка, проёмы, устройство отверстий, снятие, удаление',
  'Строительные работы': 'кладка, закладка, выравнивание, устройство перемычки, экрана, ' +
    'паро-/термо-/гидро-/звукоизоляции, установка из бетона/стеклоблока, ' +
    'заделка швов между кирпичами/стеклоблоками',
  'Разное': 'карнизы (молдинги), декор, окраска — то, что не попало в более специфичные категории',
  'Малярно-штукатурные работы (Потолки)': 'штукатурка, шпаклёвка, покраска — объект работы: потолок',
  'Малярно-штукатурные работы (Стены)': 'штукатурка, шпаклёвка, покраска, оклейка обоями — объект работы: стены',
  'Малярно-штукатурные работы (Откосы, короба, колонны)': 'штукатурка, шпаклёвка, покраска — объект работы: откосы, короба, колонны',
  'Столярно-плотницкие работы': 'работы бригады столяра-плотника: люк-невидимка, лючок, экран радиатора отопления, ' +
    'обшивка стен деревом/ГКЛ, закладные под ГКЛ, инсталляции, ниши — категория по ' +
    'специальности исполнителя, не по физическому объекту работы',
  'Установка дверей': 'всё, что связано с дверьми, КРОМЕ установки дверных коробок',
  'Прочие работы': 'вынос мусора, погрузка/разгрузка, перенос мебели, штрабы, производство нестандартных работ — узкая категория, не общий fallback для любой неопределённости',
  'Бетонные работы': 'фундамент, отмостки, сваи',
  'Плиточные работы': 'работы бригады плиточника: облицовка и укладка плиткой/керамогранитом/мозаикой/мрамором, затирка швов, фигурная резка',
  'Устройство полов': 'работы бригады по устройству полов: стяжка, наливные полы, укладка ламината/паркета/линолеума',
  'Электромонтажные работы': 'работы бригады электрика: проводка, розетки, выключатели, электрощиты, освещение, подрозетники, тв, интернет, телефония, домофоны',
  'Электромонтажные работы (Умный дом)': 'монтаж систем автоматизации дома (KNX и аналоги): умные выключатели/розетки, датчики, ' +
    'сценарии, шлюзы — делает та же бригада электрика, но специализация и цена выше, чем ' +
    'у обычного электромонтажа',
  'Вентиляционные работы': 'работы бригады вентиляционщика: монтаж воздуховодов, вентиляторов, врезка в короба, решётки',
  'Сантехнические работы': 'работы бригады сантехника: трубы водоснабжения/канализации, смесители, сантехприборы, изоляция труб, опрессовка, системы фильтрации жидкостей, установка зеркала (всё, что делается в ванной/душевой)',
  'Работы по монтажу системы отопления': 'работы бригады по отоплению: радиаторы, котлы, трубы отопления, водяной тёплый пол',
  'Устройство натяжных потолков': 'работы бригады монтажников натяжных потолков — отдельная узкая специализация, не путать с малярно-штукатурными работами по потолку',
...
async function categorize(entry) {
  ...
  try {
    ...
    const categoryResult = await askLibrarian({
      ...
      extraContext:
        'Если позиция явно не подходит ни под одну категорию — не выбирай ближайшую по касательной; ' +
        'предложи новую категорию (is_new=true), либо, только если позиция буквально про вынос мусора/' +
        'погрузку-разгрузку/перенос мебели/штрабы/нестандартные работы — "Прочие работы".',
      valueInstructionNote:
